...
public class SessionTimeoutFilter implements Filter {
	 ...
	 private String timeoutPage = "login.html";
	 ...
	 private boolean isSessionControlRequiredForThisResource(HttpServletRequest httpServletRequest) {
	  ...
	  boolean controlRequired = !requestPath.equals(getTimeoutPage());// (String)getTimeoutPage());
		...
		if ((request instanceof HttpServletRequest) && (response instanceof HttpServletResponse)) {
			   ...
			   if (isSessionControlRequiredForThisResource(httpServletRequest)) {
			    ...
			    if (isSessionInvalid(httpServletRequest)) {
			     String timeoutUrl = httpServletRequest.getContextPath() + "/index.html";
			     logger.info("session is invalid! redirecting to timeoutpage : " + timeoutUrl);
			     
			     httpServletResponse.sendRedirect(timeoutUrl);
			    // httpServletResponse.sendRedirect(request, response, "/login.html?error");
			     return;
			    }
			   }
			  }
